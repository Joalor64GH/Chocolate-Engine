...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		case 'spookeez' | 'south' | 'monster': 
			...
			default:
					defaultCamZoom = 0.9;
				curStage = 'stage';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stage/stages/stageback'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('stage/stages/stagefront'));
				...
				var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('stage/stages/stagecurtains'));
	...
	private function keyShit():Void
	{
		...
		if ((up || right || down || left || upHold || downHold || leftHold || rightHold) && generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.canBeHit && daNote.mustPress && daNote.isSustainNote)
				{
					switch (daNote.noteData % 4) // just in case
					{
	...
	function badNoteCheck()
	{
		...
		// REDO THIS SYSTEM! -idk
		// Done? -Wither
		var things:Array<Bool> = [controls.LEFT_P, controls.DOWN_P, controls.UP_P, controls.RIGHT_P];
